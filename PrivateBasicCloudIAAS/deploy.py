...
def SetNFS():
    ...
    file.close()    
    subprocess.check_output(['systemctl enable rpcbind.service nfs-server.service'], shell=True)
...
def InstallVPN():
    ...
    subprocess.check_output(['/usr/local/vpnserver/vpncmd /server localhost /password:' + vpnserverpwd + ' /adminhub:DEFAULT /cmd SecureNatHostSet /MAC:none /IP:192.168.122.1 /MASK:255.255.255.0'], shell=True)
    subprocess.check_output(['/usr/local/vpnserver/vpncmd /server localhost /password:' + vpnserverpwd + ' /adminhub:DEFAULT /cmd DhcpSet /START:192.168.122.150 /END:192.168.122.199 /MASK:255.255.255.0 /EXPIRE:7200 /GW:none /DNS:none /DNS2:none /DOMAIN:none /LOG:yes'], shell=True)
